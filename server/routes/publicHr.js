...
router.post('/employee-fill/:token', (req, res) => {
  ...
  try {
    if (employeeId) {
      ...
      const { istToday } = require('../lib/istDate');
      const { seedHiredRow } = require('../lib/employeeTimeline');
      // Blend with Workspace lifecycle: land as draft + empty satellite rows so
      // HR Statutory/Pay section saves (plain UPDATE, no upsert) have a row to hit.
      // Free-text dept/desig stay as intake; catalog bind happens later in Workspace.
      const createSelfFill = db.transaction(() => {
        const r = db.prepare(`INSERT INTO employees (name, phone, email, designation, department, join_date,
                                aadhar_file, pan_file, qualification_file, onboarding_status, updated_at)
                              VALUES (?,?,?,?,?,?,?,?,?,?, datetime('now'))`)
          .run(vals.name, vals.phone, vals.email || '', vals.designation || '', vals.department || '',
               vals.join_date || istToday(), vals.aadhar_file, vals.pan_file, vals.qualification_file, 'draft');
        const id = r.lastInsertRowid;
        db.prepare('INSERT INTO employee_statutory (employee_id) VALUES (?)').run(id);
        db.prepare('INSERT INTO employee_compensation (employee_id) VALUES (?)').run(id);
        seedHiredRow(db, {
          employeeId: id,
          effectiveFrom: vals.join_date || istToday(),
          source: 'self_fill',
          changedBy: link.created_by || null,
        });
        return id;
      });
      employeeId = createSelfFill();
    }
