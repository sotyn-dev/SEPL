...
const { buildQuotationBuffer } = require('../jobs/lib/quotationExcel');
...
router.post('/estimate-export', async (req, res) => {
  ...
  // Group by category once — the normalized input both export paths take.
  const byCat = {};
  for (const r of rows) { const c = r.category || 'General'; (byCat[c] = byCat[c] || []).push(r); }
  const data = { title, client_name, client_address, quotation_no, prep_by, byCat, manpower };
  const sendBuf = (buf) => {
    res.setHeader('Content-Disposition', `attachment; filename="quotation-${String(title || 'estimate').replace(/[^a-z0-9]/gi, '_')}.xlsx"`);
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.send(buf);
  };
  // Off-loop the CPU-heavy workbook build to the background worker when a live
  // worker + Redis are available (keeps the API event loop free on the 2-core
  // VPS); the client sees the SAME streamed .xlsx either way. If the queue is
  // unavailable OR the job errors, build inline right here — unchanged behavior.
  try {
    const buf = await jobs.runFileJob(jobs.QUEUES.FILES, jobs.JOBS.EXCEL_EXPORT_QUOTATION, data);
    if (buf) return sendBuf(buf);            // built by the worker, off the API loop
  } catch (e) { /* fall through to the inline build below */ }
  try {
    return sendBuf(await buildQuotationBuffer(data));   // inline fallback (same shared builder)
  } catch (err) {
