...
export default function Collections() {
  ...
  // Snap back to page 1 whenever the status filter changes, so the user
  ...
  // Client-side pagination over `receivables` (already status-filtered
  // server-side inside load()).
  const total = receivables.length;
  const totalPages = Math.max(1, Math.ceil(total / pageSize));
  const curPage = Math.min(page, totalPages);
  const from = total === 0 ? 0 : (curPage - 1) * pageSize;
  const to = Math.min(from + pageSize, total);
  const pagedRows = receivables.slice(from, to);
