          ...
          <tbody>
            {rows.map(r => (
              <tr key={r.id} className="border-b hover:bg-gray-50 cursor-pointer" onClick={() => setSelected(r)}>
                <td className="px-2 py-1.5 whitespace-nowrap">{new Date(r.at).toLocaleString()}</td>
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-4 border-b sticky top-0 bg-white">
              <div>
                ...
                <p className="text-xs text-gray-500"><FiCalendar className="inline mr-1" size={11} /> {new Date(selected.at).toLocaleString()}</p>
              </div>
