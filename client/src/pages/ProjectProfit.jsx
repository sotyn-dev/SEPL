...
export default function ProjectProfit() {
  ...
  const open=p=>{setSelected(p.id);setForm(initial());setSaveError('');setVoidId(null);};
  const save=async e=>{
    e.preventDefault();setSaving(true);setSaveError('');
    try {await api.post('/project-profit/adjustments',{...form,project_id:selected,basis,amount:Number(form.amount)});setForm(initial());setNotice('Adjustment saved. Totals refreshed.');setRefresh(n=>n+1);}
  ...
  return <main className="ppl">
    ...
    <details className="ppl-method"><summary>How these totals are calculated</summary>
      <p><strong>Profit / loss = revenue − costs.</strong> Each row is one Business Book order; similarly named orders are kept separate. Contract value is context only and is never counted as earned revenue.</p>
