...
export default function HelpTickets() {
  ...
  // Pagination — tickets table below.
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);
  // Tracks the last filter combo we paginated for, so a change can reset
  // `page` to 1. Adjusted during render (React's documented pattern for
  // "reset state when inputs change") instead of a useEffect, since an
  // effect here would just cause an extra render pass.
  const [prevPageFilterKey, setPrevPageFilterKey] = useState(`${scope}|${statusFilter}|${search}|${nameFilter}`);
  ...
  // Reset to page 1 whenever the scope tab, status filter, name filter or
  // search text changes.
  const pageFilterKey = `${scope}|${statusFilter}|${search}|${nameFilter}`;
  if (pageFilterKey !== prevPageFilterKey) {
    setPrevPageFilterKey(pageFilterKey);
    setPage(1);
  }

  // Client-side pagination — over `filtered`, so it composes with the
  // scope tab, status dropdown, name filter and search box above.
  const total = filtered.length;
  const totalPages = Math.max(1, Math.ceil(total / pageSize));
  const curPage = Math.min(page, totalPages);
  const from = total === 0 ? 0 : (curPage - 1) * pageSize;
  const to = Math.min(from + pageSize, total);
  const pagedTickets = filtered.slice(from, to);
    ...
    <div className="space-y-4">
      ...
      <div className="card p-0 overflow-auto max-h-[70vh]">
        <table className="text-sm w-full">
          ...
          <tbody>
            {total === 0 && <tr><td colSpan="10" className="text-center py-8 text-gray-400 text-sm">No tickets {scope === 'mine' ? 'assigned to you' : scope === 'given' ? 'raised by you' : ''} yet.</td></tr>}
            {pagedTickets.map(t => {
