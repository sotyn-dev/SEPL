...
export default function ProofPreview({ url, name, type, reviewOnly = false }) {
  ...
  return (
    <div className="mt-3 rounded-lg border border-gray-200 bg-gray-50 p-3 space-y-2">
      <p className="text-sm font-semibold text-gray-800">{reviewOnly ? 'Submitted proof' : 'Check your proof before submitting'}</p>
      ...
      {!reviewOnly && <p className="text-xs text-gray-500">Wrong file? Choose another file or take a new photo above.</p>}
    </div>
  );
}
